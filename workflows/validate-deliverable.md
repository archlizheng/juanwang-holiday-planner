...
<process>
1. **Run validation scripts**: `node scripts/validate.mjs <html>` (static invariants) and `node scripts/smoke-test.mjs <html>` (runs real template logic: today mapping, streak, review merge, progress day). Run both on the template and on any generated personalized `.html` file. Fix all reported errors before delivery.
...
4. **Browser smoke test (required before delivery)**: actually open the generated HTML in a browser (or headless Chrome with `--virtual-time-budget` so the fadeIn animation settles), and confirm:
   ...
   - sync-to-HTML download works without fetch (including `file://`) and uses `{fileNameBase}-v{timestamp}.html`
